...
def btn_pressed(btn_id, state):
    ...
    global _to_show_binary, _to_get_pixel, _focus_mode, _btn_id_binary, _btn_id_pixel, _btn_id_focus, _gui_ref
    ...
    if btn_id == _btn_id_binary:
        ...
        if _gui_ref is not None:
            _gui_ref.setItemActive(_btn_id_focus, _focus_mode)
...
def main():
    ...
    while not app.need_exit():
        ...
        if len(blobs) > 0 and not _focus_mode:
        ...
        for i, blob in enumerate(blobs):
            ...
            if check_edge_clip(blob_rect):
                if not _focus_mode:
                    ...
                    img_display.draw_cross(blob.cx(), blob.cy(), image.COLOR_WHITE, 8, 2)
                    img_display.draw_string(x + w + 2, y - 8,
                                            "#{}[EDGE]".format(i), image.COLOR_WHITE)
                # 边缘目标不参与高价值评比
                continue
        ...
        # 乘区 C（迟滞锁定）：上一帧胜出者附近 ±50px 内 ×1.05
        # ============================================================
        if len(candidates) > 0:
            best = None
            best_score = 0
            ...
            for c in candidates:
                ...
                # 乘区 B：空间纵深权重（Y 轴，底部 1.5×）
                spatial = 1.0 + (cy / _image_height) * 0.5
                score *= spatial

                # 乘区 B2：X 轴中心加权（越靠近 160 越高，1.0~1.15×）
                center_x = _image_width // 2
                x_bonus = 1.15 - abs(cx - center_x) / center_x * 0.15
                score *= x_bonus

                # 乘区 C：迟滞锁定（仅保留 5% 微弱防抖）
                if last_target_cx >= 0:
            ...
            if best is not None:
                ...
                # 专注模式：红色十字 + 四角折线虚框
                if _focus_mode:
                    img_display.draw_cross(cx, cy, image.COLOR_RED, 12, 2)
                    pad = 4
                    arm = max(4, r // 3)
                    # 左上
                    img_display.draw_line(cx-r-pad, cy-r-pad+arm, cx-r-pad, cy-r-pad, image.COLOR_RED, 2)
                    img_display.draw_line(cx-r-pad, cy-r-pad, cx-r-pad+arm, cy-r-pad, image.COLOR_RED, 2)
                    # 右上
                    img_display.draw_line(cx+r+pad-arm, cy-r-pad, cx+r+pad, cy-r-pad, image.COLOR_RED, 2)
                    img_display.draw_line(cx+r+pad, cy-r-pad, cx+r+pad, cy-r-pad+arm, image.COLOR_RED, 2)
                    # 左下
                    img_display.draw_line(cx-r-pad, cy+r+pad-arm, cx-r-pad, cy+r+pad, image.COLOR_RED, 2)
                    img_display.draw_line(cx-r-pad, cy+r+pad, cx-r-pad+arm, cy+r+pad, image.COLOR_RED, 2)
                    # 右下
